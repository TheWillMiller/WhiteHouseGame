...
## Outdoor human scale, walkable residence steps and follow camera

The shipped residence measures 50.22 game units across and 21.48 high, close to the White House Historical Association's 51.2 m length and 21.3 m height. The former 3-unit outdoor avatar was therefore oversized. Trump and outdoor staff now use a 1.9/3 root scale; walking speed, animation stride rate, eye height, follow distance and close-camera hiding are adjusted together. Indoor coordinates retain their existing authored proportions; this is not a claim that every room has measured dimensions.

`scripts/prepare-residence-terrain.mjs` decodes the shipped GLB, applies node transforms and rasterizes upward-facing low surfaces into two 10 cm height fields. The resulting run-length encoded source is about 30 KB before compression and needs no runtime mesh raycasts for walking. The center south ground-level passage stays open. North/south stair treads, landings and raised side surfaces support the player, reject rises above 30 cm, and support jumping/landing at the current elevation. Oversized flat stair-blocking rectangles were removed; camera proxies include baked step heights. Landing markers lead to the State Floor through existing verified travel spawns.

The third-person camera still uses camera-controls for damped orbit and obstruction handling. `ThirdPersonOrbit` separates a held movement gesture from automatic camera yaw so recentering cannot steer a held side/back direction into a circle. Dragging or Q/R takes priority; after 1.2 seconds without look input, a moving player's camera eases behind travel direction. Standing free look remains fixed. The outside view uses 60-degree FOV and a closer 4.8-unit follow distance, while vertical step motion is smoothed independently of the horizontal anchor. Indoor pitch is bounded by the actual ceiling and floor rather than a narrow look-angle clamp.

Validation: world connectivity, `tests/traversal-camera.test.mjs` (four real-geometry stair routes, elevated jump/landing, scale and orbit behavior), the existing camera/Oval/grounds/room suites and TypeScript. Tests run against geometry and controller code without WebGL; they do not measure real-device FPS or establish a visually verified GTA-equivalent camera.
