...
for (const file of ['world-data', 'game', 'webmcp']) {
 ...
 const built = ts.transpileModule(source, { compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ESNext } }).outputText.replace(/from ['"]\.\/world-data['"]/g, "from './world-data.mjs'");
