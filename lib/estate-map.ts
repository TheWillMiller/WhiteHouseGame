...
export const ESTATE_MAP={x:-132,z:-103,width:264,depth:242};
...
export function renderEstateMap(renderer:T.WebGLRenderer,scene:T.Scene,hidden:T.Object3D[]):string{
 const width=640,height=Math.round(width*242/264),target=new T.WebGLRenderTarget(width,height,{depthBuffer:true});target.texture.colorSpace=T.SRGBColorSpace;
 const camera=new T.OrthographicCamera(-132,132,121,-121,1,500);camera.position.set(0,280,18);camera.up.set(0,0,-1);camera.lookAt(0,0,18);camera.updateMatrixWorld();
