...
type Hotspot={id:string;label:string;kind:'npc'|'door'|'seat'|'lectern'|'equipment';pose?:[number,number,number];x:number;y?:number;z:number;target?:string;zone?:Place;spawn?:[number,number]};
...
export function grounds():World{const w:World={group:new T.Group(),solids:[],spots:[],actors:[],rings:[]},g=w.group;
 ...
 w.cameraOnly=residenceStepCamera();
...
export class Game{
 ...
 let dragging=false,px=0,py=0;listen(canvas,'pointerdown',((e:PointerEvent)=>{if(this.paused)return;dragging=true;this.orbit.looking=true;this.orbit.manualLook();px=e.clientX;py=e.clientY;canvas.setPointerCapture(e.pointerId);canvas.focus();}) as EventListener);listen(canvas,'pointermove',((e:PointerEvent)=>{if(!dragging||this.paused)return;this.orbit.manualLook();this.yaw+=(e.clientX-px)*.006;this.pitch=T.MathUtils.clamp(this.pitch+(e.clientY-py)*.003,this.firstPerson?-.75:-.45,this.firstPerson?.75:.72);px=e.clientX;py=e.clientY;}) as EventListener);for(const type of ['pointerup','pointercancel','lostpointercapture'])listen(canvas,type,()=>{dragging=false;this.orbit.looking=false;this.orbit.manualLook();});listen(canvas,'contextmenu',e=>e.preventDefault());listen(canvas,'wheel',((e:WheelEvent)=>{if(this.paused)return;e.preventDefault();this.zoom=T.MathUtils.clamp(this.zoom+e.deltaY*.001,.65,1.35);}) as EventListener,{passive:false});const resize=()=>{this.camera.aspect=this.mount.clientWidth/this.mount.clientHeight;this.camera.updateProjectionMatrix();this.renderer.setSize(this.mount.clientWidth,this.mount.clientHeight);};const ro=new ResizeObserver(resize);ro.observe(this.mount);this.cleanup.push(()=>ro.disconnect());resize();
 }
 ...
 async loadCrew(){for(const [i,x,z]of [[0,72,-2],[1,58,-37.5]]){const staff=await this.staffModels.create(window.location.pathname,'foreman');if(!staff)return;if(this.disposed){staff.dispose();return;}staff.object.position.set(x,0,z);staff.object.scale.multiplyScalar(OUTDOOR_HUMAN_SCALE);staff.object.rotation.y=i?-.6:2.2;this.worlds.get('grounds')!.group.add(staff.object);const arm=staff.object.getObjectByName('Bip01_R_Forearm');this.crew.push({staff,arm,base:arm?.quaternion.clone()??new T.Quaternion(),time:i*2});}}
 ...
 emote(name:Emote|null){this.stopActivity();if(this.player.position.y<=this.floorAt(this.player.position.x,this.player.position.z)+.01){this.keys.clear();this.playerModel?.emote(name);}}
 key(code:string,on:boolean){if(on&&!this.paused)this.keys.add(code);else this.keys.delete(code);}
 toggleCamera(){this.firstPerson=!this.firstPerson;this.pitch=this.firstPerson?0:.16;this.orbit.reset(this.yaw);this.followCamera.reset();this.publish();}
 focus(){this.renderer.domElement.focus();}
 pause(value:boolean){this.paused=value;this.keys.clear();}
 jump(){this.stopActivity();if(!this.paused&&this.player.position.y<=this.floorAt(this.player.position.x,this.player.position.z)+.01){this.playerModel?.emote(null);this.velocityY=this.zone==='grounds'?4.6:6.5;}}
 floorAt(x:number,z:number){return this.zone==='grounds'?residenceHeight(x,z):0;}
 blocked(x:number,z:number){if(this.zone==='grounds'&&(x<-98||x>98||z<-95||z>126))return true;if(this.zone==='west'&&!insideWest(x,z))return true;if(this.zone!=='grounds'&&this.zone!=='west'&&(x<-28.7||x>28.7||z<-18.7||z>18.7))return true;const radius=this.zone==='grounds'?.24:.36;if(this.zone==='grounds'&&this.player){const feet=this.player.position.y;if(residenceStepBlocked(x,z,feet))return true;}return this.world.solids.some(o=>Math.abs(x-o.x)<o.w/2+radius&&Math.abs(z-o.z)<o.d/2+radius);}
 ...
 loop=(now:number)=>{if(this.disposed)return;if(document.hidden){this.last=now;this.frame=requestAnimationFrame(this.loop);return;}const elapsed=(now-(this.last||now))/1000,dt=Math.min(elapsed,.04);this.last=now;if(this.budget.sample(elapsed))this.renderer.setPixelRatio(this.budget.ratio);const p=this.player.position;const previousX=p.x,previousZ=p.z;const running=this.keys.has('ShiftLeft')||this.keys.has('ShiftRight'),slow=this.keys.has('KeyC');const mode:Locomotion=running?(slow?'sprint':'run'):(slow?'stroll':'walk');if(!this.paused){if(this.activityTime>0){this.activityTime-=dt;if(this.activityTime<=0)this.activity='';}if(this.keys.has('KeyQ')||this.keys.has('KeyR')){this.orbit.manualLook();this.yaw+=(this.keys.has('KeyR')?1:-1)*dt*1.8;}let forward=(this.keys.has('KeyW')||this.keys.has('ArrowUp')?1:0)-(this.keys.has('KeyS')||this.keys.has('ArrowDown')?1:0),right=(this.keys.has('KeyD')||this.keys.has('ArrowRight')?1:0)-(this.keys.has('KeyA')||this.keys.has('ArrowLeft')?1:0);const len=Math.hypot(forward,right),basis=this.firstPerson?this.yaw:this.orbit.movementBasis(`${forward},${right}`,this.yaw);if(len){if(this.activePose)this.stopActivity();forward/=len;right/=len;const speed=({walk:3.2,stroll:1.6,run:6.5,sprint:9}[mode])*(this.zone==='grounds'?OUTDOOR_HUMAN_SCALE:.78);const dx=(Math.sin(basis)*forward+Math.cos(basis)*right)*speed*dt,dz=(-Math.cos(basis)*forward+Math.sin(basis)*right)*speed*dt;if(!this.blocked(p.x+dx,p.z))p.x+=dx;if(!this.blocked(p.x,p.z+dz))p.z+=dz;const target=-Math.atan2(dx,-dz);this.player.rotation.y+=Math.atan2(Math.sin(target-this.player.rotation.y),Math.cos(target-this.player.rotation.y))*Math.min(1,dt*13);this.walkTime+=dt*speed*1.8;}const groundY=this.floorAt(p.x,p.z);this.velocityY-=dt*17;p.y=Math.max(groundY,p.y+this.velocityY*dt);if(this.zone!=='grounds'&&p.y>1.15){p.y=1.15;this.velocityY=Math.min(0,this.velocityY);}if(p.y===groundY)this.velocityY=0;
 ...
 if(!this.paused)this.playerModel?.animate(Math.hypot(p.x-previousX,p.z-previousZ)/(this.zone==='grounds'?OUTDOOR_HUMAN_SCALE:1),dt,p.y>this.floorAt(p.x,p.z)+.05,mode);
 const moving=Math.hypot(p.x-previousX,p.z-previousZ)>.0001;
 if(!this.paused&&!this.firstPerson)this.yaw=this.orbit.follow(this.yaw,-this.player.rotation.y,moving,dt);
 const indoors=this.zone!=='grounds',humanScale=indoors?1:OUTDOOR_HUMAN_SCALE,distance=(indoors?4.3:4.8)*this.zoom,pitch=this.pitch;
 ...
 const target=new T.Vector3(p.x,Math.min(this.cameraFeetY+(this.activePose?.kind==='seat'?1.35:2.05)*humanScale,indoors?3.25:100),p.z);
 ...
 if(this.firstPerson){
   this.camera.position.set(p.x,Math.min(this.cameraFeetY+(this.activePose?.kind==='seat'?2:2.7)*humanScale,indoors?3.8:100),p.z);
