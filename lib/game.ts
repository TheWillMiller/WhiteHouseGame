...
import { loadResidenceModel, disposeResidence } from './residence-model';
...
type Hotspot={id:string;label:string;kind:'npc'|'door';x:number;z:number;target?:string;zone?:Place;spawn?:[number,number]};
export type GameState={firstPerson?:boolean;location:string;zone:Place;x:number;z:number;heading:number;nearby:Hotspot|null;visited:string[];met:string[]};
type World={cameraOnly?:({x:number;z:number;w:number;d:number;height:number;y:number})[];cameraObstacles?:CameraObstacles;group:T.Group;solids:Obstacle[];spots:Hotspot[];actors:{group:T.Group;data:NPC;staff?:StaffActor;loading?:Promise<void>}[];rings:T.Mesh[]};
...
function ring(w:World,spot:Hotspot){w.spots.push(spot);const m=new T.Mesh(new T.TorusGeometry(1.0,.055,6,32),new T.MeshBasicMaterial({color:spot.kind==='npc'?0xe7cf8c:0x99dacf,transparent:true,opacity:.85}));m.rotation.x=Math.PI/2;m.position.set(spot.x,.10,spot.z);m.userData.dynamic=true;w.group.add(m);w.rings.push(m);}

export function grounds():World{const w:World={group:new T.Group(),solids:[],spots:[],actors:[],rings:[]},g=w.group;
 box(g,0,-.35,14,450,.6,450,0x6c8745);box(g,0,-.035,16,200,.06,225,0x547249);
 ...
 m.material.side=T.DoubleSide;g.add(m);};ellipse(0,56,61,46,5);ellipse(0,-63,38,22,5);
 box(g,0,.03,15,11,.04,45,C.path);box(g,0,.025,-48,10,.05,24,C.path);box(g,0,.025,112,8,.05,24,C.path);box(g,-86,.025,16,5,.05,175,C.path);box(g,88,.025,16,5,.05,175,C.path);box(g,0,.025,-88,173,.05,5,C.path);box(g,0,.025,111,172,.05,5,C.path);box(g,-58,.025,19,44,.05,6,C.path);box(g,56,.025,24,56,.05,5,C.path);
 ...
 for(let i=0;i<48;i++){const x=-98+i*4.1;for(const z of [-97,128]){box(g,x,1.05,z,.13,2.1,.13,0x2e4140);}box(g,x,1.6,-97,4.1,.09,.1,0x2e4140);box(g,x,1.6,128,4.1,.09,.1,0x2e4140);}for(let i=0;i<55;i++){for(const x of [-100,100]){const z=-97+i*4.1;box(g,x,1.05,z,.13,2.1,.13,0x2e4140);box(g,x,1.6,z,.1,.09,4.1,0x2e4140);}}
 // Ballroom construction, based on the published East Wing location.
 box(g,61,.03,-17,42,.12,48,0xaf9771);box(g,62,.13,-21,34,.24,34,0xc9c6b6);
 for(const x of [46,54,62,70,78])for(const z of [-37,-21,-5]){box(g,x,5.1,z,.38,10,.38,0x5f6965);solid(w,x,z,.48,.48);box(g,x,10.2,-21,.4,.45,33,0x586562);}
 for(const z of [-37,-21,-5]){box(g,62,10.2,z,33,.4,.4,0x61706a);box(g,62,6,z,33,.3,.3,0x64716c);}
 for(let i=0;i<4;i++){box(g,46+i*8,10.25,-13,.26,.26,16,0x89928a);box(g,46+i*8,10.25,-29,.26,.26,16,0x89928a);}
 const crane=new T.Group();crane.position.set(83,0,-32);for(const x of [-.7,.7])for(const z of [-.7,.7])box(crane,x,16,z,.22,32,.22,0xc79532);for(let y=1;y<32;y+=2){box(crane,0,y,-.7,1.6,.15,.15,0xe0aa37);box(crane,0,y,.7,1.6,.15,.15,0xe0aa37);const a=box(crane,0,y+.8,-.7,1.8,.12,.12,0xd8a538);a.rotation.z=.85;}box(crane,-8,31.3,0,32,.4,1.4,0xe4b548);box(crane,4,30.2,0,4,2.1,2.7,0x56676a);box(crane,-2,30,0,2,1.8,1.7,0xc8952f);box(crane,-2,30.2,-.9,1.65,1.1,.1,0x456473);cyl(crane,-16,22,0,.04,18,0x4d514b,6);box(crane,-16,13,0,1,.35,1,0xc2a253);g.add(crane);solid(w,83,-32,2,2);
...
export class Game{
 residence:T.Group|null=null;
 followCamera=new FollowCamera();firstPerson=false;zoom=1;
 ...
 async loadArchitecture(){const model=await loadResidenceModel(window.location.pathname);if(this.disposed){disposeResidence(model);return;}const grounds=this.worlds.get('grounds')!,fallback=grounds.group.getObjectByName('Residence loading fallback');if(fallback){grounds.group.remove(fallback);fallback.traverse(o=>{if(o instanceof T.Mesh)o.geometry.dispose();});}this.residence=model;grounds.group.add(model);this.renderer.shadowMap.needsUpdate=true;}
 ...
 listen(window,'keydown',((e:KeyboardEvent)=>{const target=e.target as HTMLElement;if(target.matches('input,textarea,select')||target.isContentEditable||target.closest('[role=dialog]')||(e.code==='Space'&&target.tagName==='BUTTON'))return;if(e.code==='KeyM'&&!e.repeat){e.preventDefault();this.map();return;}if(this.paused)return;if(e.code==='KeyF'&&!e.repeat){e.preventDefault();this.toggleCamera();return;}if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','KeyW','KeyA','KeyS','KeyD'].includes(e.code))e.preventDefault();this.keys.add(e.code);if(e.code==='KeyE'&&!e.repeat)this.interact();if(e.code==='Space'&&!e.repeat)this.jump();if(!e.repeat){const moves:Record<string,Emote>={KeyG:'dance',KeyY:'ymca',KeyV:'victory',KeyB:'backflip'};if(moves[e.code])this.emote(moves[e.code]);if(e.code==='Escape')this.emote(null);}}) as EventListener);
 listen(window,'keyup',((e:KeyboardEvent)=>{this.keys.delete(e.code);}) as EventListener);listen(window,'blur',()=>this.keys.clear());listen(document,'visibilitychange',()=>{this.keys.clear();if(document.hidden)void this.audioContext?.suspend();else if(this.gain&&this.gain.gain.value>0)void this.audioContext?.resume();});
 let dragging=false,px=0,py=0;listen(canvas,'pointerdown',((e:PointerEvent)=>{if(this.paused)return;dragging=true;px=e.clientX;py=e.clientY;canvas.setPointerCapture(e.pointerId);canvas.focus();}) as EventListener);listen(canvas,'pointermove',((e:PointerEvent)=>{if(!dragging||this.paused)return;this.yaw+=(e.clientX-px)*.006;this.pitch=T.MathUtils.clamp(this.pitch+(e.clientY-py)*.003,this.firstPerson?-.75:-.08,this.firstPerson?.75:.48);px=e.clientX;py=e.clientY;}) as EventListener);for(const type of ['pointerup','pointercancel','lostpointercapture'])listen(canvas,type,()=>{dragging=false;});listen(canvas,'contextmenu',e=>e.preventDefault());listen(canvas,'wheel',((e:WheelEvent)=>{if(this.paused)return;e.preventDefault();this.zoom=T.MathUtils.clamp(this.zoom+e.deltaY*.001,.65,1.35);}) as EventListener,{passive:false});const resize=()=>{this.camera.aspect=this.mount.clientWidth/this.mount.clientHeight;this.camera.updateProjectionMatrix();this.renderer.setSize(this.mount.clientWidth,this.mount.clientHeight);};const ro=new ResizeObserver(resize);ro.observe(this.mount);this.cleanup.push(()=>ro.disconnect());resize();
 }
 addNPCs(w:World,zone:Place){for(const data of npcs.filter(n=>n.zone===zone)){const actor=new T.Group();actor.name=data.name;actor.position.set(data.x,0,data.z);w.group.add(actor);const tag=label(actor,data.name,0,3.35,0,2.4);tag.userData.npcLabel=true;w.actors.push({group:actor,data});ring(w,{id:data.id,label:data.name,kind:'npc',x:data.x,z:data.z});}}
 async loadStaff(){this.staffEnabled=true;const w=this.world;await Promise.all(w.actors.map(actor=>{if(actor.staff)return Promise.resolve();if(actor.loading)return actor.loading;actor.loading=this.staffModels.create(window.location.pathname,actor.data.id).then(model=>{if(!model)return;actor.staff=model;actor.group.add(model.object);this.renderer.shadowMap.needsUpdate=true;}).finally(()=>{actor.loading=undefined;});return actor.loading;}));}
 emote(name:Emote|null){if(this.player.position.y<=.01){this.keys.clear();this.playerModel?.emote(name);}}
 key(code:string,on:boolean){if(on&&!this.paused)this.keys.add(code);else this.keys.delete(code);}
 toggleCamera(){this.firstPerson=!this.firstPerson;this.pitch=this.firstPerson?0:.16;this.followCamera.reset();this.publish();}
 focus(){this.renderer.domElement.focus();}
 pause(value:boolean){this.paused=value;this.keys.clear();}
 jump(){if(!this.paused&&this.player.position.y<=.01){this.playerModel?.emote(null);this.velocityY=6.5;}}
 blocked(x:number,z:number){if(this.zone==='grounds'&&(x<-98||x>98||z<-95||z>126))return true;if(this.zone==='west'&&!insideWest(x,z))return true;if(this.zone!=='grounds'&&this.zone!=='west'&&(x<-28.7||x>28.7||z<-18.7||z>18.7))return true;return this.world.solids.some(o=>Math.abs(x-o.x)<o.w/2+.36&&Math.abs(z-o.z)<o.d/2+.36);}
 ...
 change(zone:Place,x:number,z:number){this.renderer.shadowMap.needsUpdate=true;this.playerModel?.emote(null);if(zone!==this.zone){this.scene.remove(this.world.group);if(!this.worlds.has(zone)){const w=interior(zone);this.addNPCs(w,zone);this.worlds.set(zone,w);}this.zone=zone;this.world=this.worlds.get(zone)!;this.scene.add(this.world.group);}if(this.staffEnabled)void this.loadStaff().catch(error=>console.warn('Staff could not load; re-enter the room to retry.',error));this.player.position.set(x,0,z);this.player.visible=true;this.followCamera.reset();this.camera.fov=zone==='grounds'?52:66;this.camera.updateProjectionMatrix();this.yaw=0;this.pitch=.16;this.velocityY=0;this.keys.clear();this.nearby=null;this.scene.background=new T.Color(zone==='grounds'?0xa4cee0:0xabb9b7);this.scene.fog=zone==='grounds'?new T.Fog(0xa4cee0,140,360):null;this.hemi.intensity=zone==='grounds'?1.85:1.7;this.hemi.groundColor.setHex(zone==='grounds'?0x596142:0x817158);this.sun.intensity=zone==='grounds'?2.15:1.8;this.camera.position.set(x,5,z+9);this.publish();}
 interact(){if(this.paused||!this.nearby)return;const n=this.nearby;if(n.kind==='npc'){this.keys.clear();this.met.add(n.id);this.publish();this.talk(n.id);}else if(n.target)this.travel(n.target);else if(n.zone&&n.spawn)this.change(n.zone,...n.spawn);}
 publish(){const p=this.player.position;let location=this.zone==='grounds'?'White House grounds':this.zone==='west'?'West Wing corridor':'Residence corridor';for(const d of destinations.filter(d=>d.zone===this.zone)){const inRoom=d.room?Math.abs(p.x-d.x)<d.room.w/2&&Math.abs(p.z-d.z)<d.room.d/2:Math.hypot(p.x-d.x,p.z-d.z)<(d.id==='south'?30:d.id==='north'?30:14);if(inRoom){location=d.name;this.visited.add(d.id);break;}}this.update({firstPerson:this.firstPerson,location,zone:this.zone,x:p.x,z:p.z,heading:this.yaw,nearby:this.nearby,visited:[...this.visited],met:[...this.met]});}
 loop=(now:number)=>{if(this.disposed)return;if(document.hidden){this.last=now;this.frame=requestAnimationFrame(this.loop);return;}const elapsed=(now-(this.last||now))/1000,dt=Math.min(elapsed,.04);this.last=now;if(this.budget.sample(elapsed))this.renderer.setPixelRatio(this.budget.ratio);const p=this.player.position;const previousX=p.x,previousZ=p.z;const running=this.keys.has('ShiftLeft')||this.keys.has('ShiftRight'),slow=this.keys.has('KeyC');const mode:Locomotion=running?(slow?'sprint':'run'):(slow?'stroll':'walk');if(!this.paused){if(this.keys.has('KeyQ'))this.yaw-=dt*1.8;if(this.keys.has('KeyR'))this.yaw+=dt*1.8;let forward=(this.keys.has('KeyW')||this.keys.has('ArrowUp')?1:0)-(this.keys.has('KeyS')||this.keys.has('ArrowDown')?1:0),right=(this.keys.has('KeyD')||this.keys.has('ArrowRight')?1:0)-(this.keys.has('KeyA')||this.keys.has('ArrowLeft')?1:0);const len=Math.hypot(forward,right);if(len){forward/=len;right/=len;const speed=({walk:3.2,stroll:1.6,run:6.5,sprint:9}[mode])*(this.zone==='grounds'?1:.78);const dx=(Math.sin(this.yaw)*forward+Math.cos(this.yaw)*right)*speed*dt,dz=(-Math.cos(this.yaw)*forward+Math.sin(this.yaw)*right)*speed*dt;if(!this.blocked(p.x+dx,p.z))p.x+=dx;if(!this.blocked(p.x,p.z+dz))p.z+=dz;const target=-Math.atan2(dx,-dz);this.player.rotation.y+=Math.atan2(Math.sin(target-this.player.rotation.y),Math.cos(target-this.player.rotation.y))*Math.min(1,dt*13);this.walkTime+=dt*speed*1.8;}this.velocityY-=dt*17;p.y=Math.max(0,p.y+this.velocityY*dt);if(this.zone!=='grounds'&&p.y>1.15){p.y=1.15;this.velocityY=Math.min(0,this.velocityY);}if(p.y===0)this.velocityY=0;
 let near:Hotspot|null=null,best=3.5;for(const spot of this.world.spots){const distance=Math.hypot(p.x-spot.x,p.z-spot.z);if(distance<best){near=spot;best=distance;}}this.nearby=near;for(const a of this.world.actors){const distance=a.group.position.distanceTo(p);if(distance<7){const target=-Math.atan2(p.x-a.group.position.x,-(p.z-a.group.position.z));a.group.rotation.y+=Math.atan2(Math.sin(target-a.group.rotation.y),Math.cos(target-a.group.rotation.y))*Math.min(1,dt*3);}a.group.visible=distance<65;if(distance<24)a.staff?.animate(dt);a.group.position.y=0;}}
 if(!this.paused)this.playerModel?.animate(Math.hypot(p.x-previousX,p.z-previousZ),dt,p.y>.01,mode);
 const indoors=this.zone!=='grounds',distance=(indoors?4.7:8.2)*this.zoom,pitch=indoors?T.MathUtils.clamp(this.pitch,-.08,.30):this.pitch;
 const target=new T.Vector3(p.x,Math.min(p.y+2.05,indoors?3.25:100),p.z);
 ...
 if(indoors)desired.y=Math.min(3.9,desired.y);
 ...
 if(this.firstPerson){
   this.camera.position.set(p.x,Math.min(p.y+2.7,indoors?3.8:100),p.z);
   ...
   const follow=this.followCamera.solve(target,desired,surfaces,dt,this.camera);this.camera.position.copy(follow.position);this.player.visible=follow.showPlayer;this.camera.lookAt(target);
 }
...
function furnish(w:World,d:Destination){const g=w.group,{x,z}=d,r=d.room!,{style}=r;const rx=r.w/2,rz=r.d/2;
 ...
 wall(w,x,north,r.w,false,r.color,r.doors?r.doors.north??[]:[x],1);wall(w,x,south,r.w,false,r.color,r.doors?r.doors.south??[]:[x],-1);
 ...
 chandelier(g,x,z);
 if(style==='office'){
   rug(g,x,z,rx*.7,rz*.6,0x61736d);desk(w,x,z+rz-3,Math.min(r.w-3,3.8));chair(g,x,z+rz-1.6,0x4b3b2d);lamp(g,x-1.25,1.32,z+rz-2.7);chair(g,x-1,z+1,0x716958);chair(g,x+1,z+1,0x716958);plant(g,x-rx+1.2,0,z+rz-1.4);flag(g,x+rx-1.2,0,z+rz-1.2,.53);officeStorage(w,d);
   ...
   const podiumX=x+rx-3;box(g,podiumX,.13,z,3.5,.24,r.d-5,0x334b61);box(g,podiumX,.85,z,.9,1.5,.8,0x42536b);box(g,podiumX,1.65,z,1.3,.12,1,C.wood);box(g,x+rx-.4,2.4,z,.12,4,r.d-5,0x284f70);solid(w,podiumX,z,1.3,1);for(let row=0;row<5;row++)for(const offset of [-4.5,-2.7,2.7,4.5]){const px=x-rx+3+row*3;chair(g,px,z+offset,0x315576,-Math.PI/2);solid(w,px,z+offset,.95,.95);}flag(g,podiumX,0,z+4,.55);
 }else if(style==='bedroom'){
   rug(g,x,z,rx*.7,rz*.65,0x7d7773);box(g,x,0.65,z+1,3.5,.75,4.4,C.wood);box(g,x,1.1,z+1,3.4,.25,4.2,0xf1e7cd);box(g,x,1.28,z+1.6,3.3,.16,2.6,0xa3afa0);box(g,x,1.8,z-1.2,3.6,1.8,.2,C.wood);for(const side of [-1,1]){box(g,x+side*.85,1.3,z-.5,1.2,.23,.8,C.white);cyl(g,x+side*1.75,1.5,z-1.2,.1,3,C.wood);desk(w,x+side*(rx-1.4),z-1,1.3,1.1);}solid(w,x,z+1,3.5,4.4);plant(g,x-rx+1.2,0,z+rz-1.3);
   ...
   for(const sx of [-11,11]){cyl(g,x+sx,2.1,z,.34,4.2,C.white,12);plant(g,x+sx,0,z+3.5);}rug(g,x,z,6,4,0xa24f48);
 }else{
   rug(g,x,z,rx*.71,rz*.62,style==='salon-oval'?0x41627a:0x867660,style==='salon-oval');cyl(g,x,.85,z,1.0,.15,C.wood);cyl(g,x,.42,z,.25,.85,C.wood);sofa(g,x-rx+1.6,z,Math.PI/2);sofa(g,x+rx-1.6,z,-Math.PI/2);plant(g,x-rx+1.1,0,z+rz-1.3);plant(g,x+rx-1.1,0,z+rz-1.3);
 }
}
