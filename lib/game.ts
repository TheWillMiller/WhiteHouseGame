...
type Hotspot={id:string;label:string;kind:'npc'|'door'|'seat'|'lectern'|'equipment';pose?:[number,number,number];x:number;z:number;target?:string;zone?:Place;spawn?:[number,number]};
export type GameState={estateMap?:string;activity?:string;firstPerson?:boolean;location:string;zone:Place;x:number;z:number;heading:number;nearby:Hotspot|null;visited:string[];met:string[]};
...
export function grounds():World{const w:World={group:new T.Group(),solids:[],spots:[],actors:[],rings:[]},g=w.group;
 ...
 w.worksite=buildWorksite(g);solid(w,83,-32,2,2);
 ...
 const spoil=ball(g,73,.1,-40,2.5,0x867355,1);spoil.scale.y=.4;solid(w,69,-47,3,5);ring(w,{id:'signal-crane',label:'Signal the next steel lift',kind:'equipment',x:76,z:-43});textPanel(g,'CRANE LIFT CONTROL',76,1.3,-44,2,.6);
 ...
 groundsDetails(g);gardenFinishes(g);westWingFinishes(g);for(const x of [-62.8,-57.2])solid(w,x,-41.55,.7,.7,4.6);for(const [x,z]of [[35,18]])for(const dx of [-9,9])for(const dz of [-5.5,5.5])solid(w,x+dx,z+dz,1.1,1.1,1.7);const sky=new Sky();sky.scale.setScalar(10000);sky.userData.dynamic=true;sky.userData.mapExclude=true;sky.frustumCulled=false;sky.material.uniforms.turbidity.value=3.2;sky.material.uniforms.rayleigh.value=1.5;sky.material.uniforms.mieCoefficient.value=.004;sky.material.uniforms.mieDirectionalG.value=.8;sky.material.uniforms.sunPosition.value.set(-35,60,28);g.add(sky);mergeStatic(g);return w;
}

export class Game{
 crew:{staff:StaffActor;arm:T.Object3D|undefined;base:T.Quaternion;time:number}[]=[];crewStarted=false;
 residence:T.Group|null=null;estateMap='';activity='';activityTime=0;activePose:Hotspot|null=null;returnPosition=new T.Vector3();
 ...
 async loadCrew(){for(const [i,x,z]of [[0,72,-2],[1,58,-37.5]]){const staff=await this.staffModels.create(window.location.pathname,'foreman');if(!staff)return;if(this.disposed){staff.dispose();return;}staff.object.position.set(x,0,z);staff.object.rotation.y=i?-.6:2.2;this.worlds.get('grounds')!.group.add(staff.object);const arm=staff.object.getObjectByName('Bip01_R_Forearm');this.crew.push({staff,arm,base:arm?.quaternion.clone()??new T.Quaternion(),time:i*2});}}
 ...
 emote(name:Emote|null){this.stopActivity();if(this.player.position.y<=.01){this.keys.clear();this.playerModel?.emote(name);}}
 ...
 jump(){this.stopActivity();if(!this.paused&&this.player.position.y<=.01){this.playerModel?.emote(null);this.velocityY=6.5;}}
 ...
 const target=new T.Vector3(p.x,Math.min(p.y+(this.activePose?.kind==='seat'?1.35:2.05),indoors?3.25:100),p.z);
 ...
 if(this.firstPerson){
   this.camera.position.set(p.x,Math.min(p.y+(this.activePose?.kind==='seat'?2:2.7),indoors?3.8:100),p.z);
