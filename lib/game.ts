...
export type GameState={arcade?:ArcadeState;driving?:boolean;speed?:number;estateMap?:string;activity?:string;firstPerson?:boolean;location:string;zone:Place;x:number;z:number;heading:number;nearby:Hotspot|null;visited:string[];met:string[]};
...
export class Game{
 ...
 pause(value:boolean){this.paused=value;this.keys.clear();this.stick={x:0,y:0};this.motor.reset();this.arcade?.velocity.set(0,0,0);if(value&&this.cart)this.cart.motor.reset();}
 ...
 startArcade(mode:ArcadeMode){
  ...
  this.arcade.setMode(mode);this.firstPerson=mode==='blaster'?true:mode==='flight'?false:saved;
 ...
 if(this.arcade?.mode==='flight'){
   ...
   this.player.rotation.y=-this.yaw;this.velocityY=0;
 ...
 if(!this.paused&&!this.firstPerson&&this.arcade?.mode!=='flight')this.yaw=this.orbit.follow(this.yaw,-this.player.rotation.y,moving,dt);
 ...
 if(!this.paused&&this.arcade&&this.arcade.mode!=='off'){this.arcade.update(dt);if(this.keys.has('KeyX'))this.fireBlaster();}
