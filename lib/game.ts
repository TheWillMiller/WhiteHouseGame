...
export class Game{
 ...
 if(this.arcade?.mode==='flight'){
   ...
   const cart=this.cart,racing=this.arcade?.mode==='race',canDrive=!racing||!!this.race?.ready();if(racing)this.race?.beforeDrive(dt,cart,this.keys.has('ShiftLeft')||this.keys.has('ShiftRight'),this.keys.has('KeyX'));else cart.motor.maxForwardSpeed=8;cart.update(canDrive?dt:0,T.MathUtils.clamp(forward,-1,1),T.MathUtils.clamp(right,-1,1),this.keys.has('Space'),(x,z)=>this.blocked(x,z)||this.floorAt(x,z)>.16||this.world.actors.some(a=>Math.hypot(x-a.group.position.x,z-a.group.position.z)<.5)||(Math.abs(x+72)<6.5&&Math.abs(z-50)<12.5));
