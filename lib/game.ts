...
export type GameState={driving?:boolean;speed?:number;estateMap?:string;activity?:string;firstPerson?:boolean;location:string;zone:Place;x:number;z:number;heading:number;nearby:Hotspot|null;visited:string[];met:string[]};
...
export class Game{
 cart:GolfCart|null=null;motor=new LocomotionMotor();stick={x:0,y:0};
 ...
 emote(name:Emote|null){if(this.cart?.driving)return;this.stopActivity();if(this.player.position.y<=this.floorAt(this.player.position.x,this.player.position.z)+.01){this.keys.clear();this.playerModel?.emote(name);}}
 moveStick(x:number,y:number){if(this.paused){this.stick={x:0,y:0};return;}this.stick={x,y};}
 ...
 if(this.cart?.driving){
   const cart=this.cart;cart.update(dt,T.MathUtils.clamp(forward,-1,1),T.MathUtils.clamp(right,-1,1),this.keys.has('Space'),(x,z)=>this.blocked(x,z)||this.floorAt(x,z)>.16||this.world.actors.some(a=>Math.hypot(x-a.group.position.x,z-a.group.position.z)<.5)||(Math.abs(x+72)<6.5&&Math.abs(z-50)<12.5));
   ...
   // A thumbstick gesture keeps its camera basis until released; turning the view explicitly updates it.
   const signature=this.stick.x||this.stick.y?'stick':`${forward},${right}`,basis=this.firstPerson?this.yaw:this.orbit.movementBasis(signature,this.yaw);
 ...
 const indoors=this.zone!=='grounds',humanScale=indoors?1:OUTDOOR_HUMAN_SCALE,distance=(this.cart?.driving?6.8:indoors?4.3:4.8)*this.zoom,pitch=this.pitch;
