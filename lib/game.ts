...
export class Game{
 ...
 pollController(dt:number){
  ...
  if(frame.lookX||frame.lookY){this.orbit.manualLook();this.yaw+=frame.lookX*dt*2.1;this.pitch=T.MathUtils.clamp(this.pitch+frame.lookY*dt*1.35,this.firstPerson?-.75:-.45,this.firstPerson?.75:.72);}
 ...
 if(this.arcade?.mode==='flight'){
   ...
   const cart=this.cart,racing=this.arcade?.mode==='race',canDrive=!racing||!!this.race?.ready();if(racing)this.race?.beforeDrive(dt,cart,this.down('ShiftLeft')||this.down('ShiftRight'),this.down('KeyX'));else cart.motor.maxForwardSpeed=8;cart.update(canDrive?dt:0,T.MathUtils.clamp(forward,-1,1),T.MathUtils.clamp(right,-1,1),this.down('Space'),(x,z)=>this.blocked(x,z)||this.floorAt(x,z)>.16||this.world.actors.some(a=>Math.hypot(x-a.group.position.x,z-a.group.position.z)<.5)||(Math.abs(x+72)<6.5&&Math.abs(z-50)<12.5));
