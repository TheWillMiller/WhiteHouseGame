...
const room=(id:string,name:string,left:number,north:number,right:number,south:number,style:string,doors:NonNullable<Destination['room']>['doors'],spawn:[number,number],description:string):Destination=>({id,name,short:name.match(/· (\d+)$/)?.[1]??({'west-lobby':'Lobby','press-hall':'Entrance','vp':'Vice President','chief':'Chief of Staff','private-dining':'Dining','study':'Study','roosevelt':'Roosevelt','cabinet':'Cabinet','press-secretary':'Press Secretary','press':'Briefing room','press-corps':'Press offices','colonnade':'Portrait gallery','palm':'Palm Room','reception':'Reception'} as Record<string,string>)[id],zone:'west',x:(left+right)/2,z:(north+south)/2,spawn,description,room:{w:right-left,d:south-north,color:style==='press'?0x8cacc0:style==='corridor'?0xe7e0c8:0xc9c1ad,style,doors}});
export const westDestinations:Destination[]=[
 ...
 room('colonnade','Presidential Walk of Fame',18,-25,84,-17,'corridor',{north:[25,36,48,64,78],south:[25,44],west:[-21],east:[-21]},[54,-21],'47 framed portraits in the West Colonnade, beside the Rose Garden'),
