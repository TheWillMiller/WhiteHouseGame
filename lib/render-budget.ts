...
export type Footprint={x:number;z:number;w:number;d:number;height?:number};
export class CameraObstacles {
 readonly meshes:T.Mesh[]=[];
 private records:{mesh:T.Mesh;box:Footprint;ovalWall:boolean}[]=[];
 constructor(solids:Footprint[],grounds:boolean){
  ...
  for(const box of solids){
   ...
   const mesh=new T.Mesh(geometry,material);mesh.position.set(box.x,height/2,box.z);mesh.scale.set(box.w,height,box.d);mesh.updateMatrixWorld(true);this.meshes.push(mesh);this.records.push({mesh,box,ovalWall});
  }
 }
