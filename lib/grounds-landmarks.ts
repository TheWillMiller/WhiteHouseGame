...
import {HELIPAD,SOUTH_FLAG,PRESS_TENTS,PRESS_TENT_YAW} from './grounds-layout';
...
export function buildGroundsLandmarks(parent:T.Group,solids:Solid[]):GroundsLandmarks{
 ...
 const canopyCloth=new T.MeshStandardMaterial({color:0x204f43,roughness:1,side:T.DoubleSide});
 const weatherCloth=new T.MeshStandardMaterial({color:0x434b49,roughness:1,side:T.DoubleSide});
 ...
 // Above the drive surface so the stone approach stays continuous at the crossing.
 box(parent,0,.035,(approachStart+approachEnd)/2,3.4,.032,approachEnd-approachStart,stone);
 for(let z=approachStart+1.2;z<approachEnd;z+=1.2)box(parent,0,.053,z,3.4,.003,.012,joint);
 ...
 for(const [index,[x,z]]of PRESS_TENTS.entries()){
  ...
  const obstacle=(lx:number,lz:number,w:number,d:number,height:number)=>{
   const c=Math.cos(PRESS_TENT_YAW),s=Math.sin(PRESS_TENT_YAW);
   solids.push({x:x+lx*c+lz*s,z:z-lx*s+lz*c,w:Math.abs(c)*w+Math.abs(s)*d,d:Math.abs(s)*w+Math.abs(c)*d,height});
  };
  ...
  roof.setAttribute('position',new T.Float32BufferAttribute(v,3));roof.setAttribute('uv',new T.Float32BufferAttribute(new Float32Array(v.length/3*2),2));roof.computeVertexNormals();mesh(tent,roof,canopyCloth);
  for(const sx of [-2.4,2.4])for(const sz of [-2.05,2.05]){rod(tent,[sx,.03,sz],[sx,2.65,sz],.027,aluminum);box(tent,sx,.18,sz,.34,.34,.34,black);obstacle(sx,sz,.34,.34,2.65);rod(tent,[sx,2.6,sz],[0,3.6,0],.017,aluminum);}
  for(const sz of [-2.16,2.16]){box(tent,0,2.43,sz,5,.25,.015,canopyCloth);for(const sign of [-1,1])rod(tent,[-2.4,2.43+sign*.15,sz],[2.4,2.43-sign*.15,sz],.014,aluminum);}
  for(const sx of [-2.5,2.5])box(tent,sx,2.43,0,.015,.25,4.3,canopyCloth);
  // Folded side drape and rear weather curtain leave a usable front opening.
  const curtain=mesh(tent,new T.PlaneGeometry(5,2.28,32,1),weatherCloth,0,1.28,-2.1);const cp=curtain.geometry.getAttribute('position');for(let i=0;i<cp.count;i++)cp.setZ(i,Math.sin(cp.getX(i)*15)*.035);curtain.geometry.computeVertexNormals();
  obstacle(0,-2.1,5,.08,2.5);
  // Camera points back toward the residence; a lens, hood, viewfinder, and pan bar.
  const camera=new T.Group();camera.position.set(-.9,0,.3);camera.rotation.y=-2.0-PRESS_TENT_YAW;tent.add(camera);
